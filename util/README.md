...
- [func WriteTmpFile(data string) (pathSaved string, deferCleanUp func(), err error)](<#func-writetmpfile>)
...
All functions of this package that needs to os\.Exit uses OsExit instead\. See the example of ExitOnError for how\-to\-mock\.
...
ChDirHome is similar to util\.ChDir but it moves the current working directory to the user's home directory in on\-go\. It returns a function to move back to the original directory\.
...
The input will be hashed with BLAKE3 algorithm then encodes it to Base58 \(Base58BTC\) and returns the first "len" bytes of the results\.
...
RandStr returns a random unique string of given length\. The length range must be between 1\-1024\. Otherwise it will os\.Exit with status 1\.
